...
private:
  ...
  static void playBeep(const AudioBeepConfiguration& config, std::vector<std::unique_ptr<sf::SoundBuffer>> &buffers, std::vector<std::unique_ptr<sf::Sound>> &sounds);
  static void cleanupStoppedSounds(std::vector<std::unique_ptr<sf::SoundBuffer>> &buffers, std::vector<std::unique_ptr<sf::Sound>> &sounds);
